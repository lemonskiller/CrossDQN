...
class CrossDQN:
    ...
    def create_model(self, features):
        ...
        whole_action_vector = self._SACU2(ads_emb, ois_emb)  # (?, 31, 20)
        # V, A
    ...
    def _target_attention(self, emb_map):
        ad_attention_res = self._attention(
            emb_map['ad_emb'], emb_map['behavior_emb'])
        oi_attention_res = self._attention(
            emb_map['oi_emb'], emb_map['behavior_emb'])
    ...
    def _attention(self, ori_queries, ori_keys):
        ...
        d_layer_3_all = tf.reshape(
            d_layer_3_all, [-1, tf.shape(queries)[1], tf.shape(queries)[2]])
    ...
    def _SACU(self, ads_emb, ois_emb, action):
        ...
        cross_fea = tf.matmul(tf.cast(ad_cum_index_onehot, tf.float32), ads_emb) * tf.expand_dims(
            ...
            tf.cast(nature_cum_index_onehot, tf.float32), ois_emb) * tf.expand_dims(tf.cast(nature_index, tf.float32),
                                                                                    axis=2)  # batch * 5
        return tf.reshape(cross_fea, [-1, ACTION_LEN * CHANNEL_CNT])
    ...
    def _SACU2(self, ads_emb, ois_emb):
        ...
        ad_cum_index_onehot = tf.one_hot(tf.where(ad_cum_index < 0, tf.zeros_like(ad_cum_index), ad_cum_index),
                                         depth=5)  # 32 * 5 * 5
        nature_cum_index_onehot = tf.one_hot(
            tf.where(nature_cum_index < 0, tf.zeros_like(
                ad_cum_index), nature_cum_index),
            depth=5)  # 32 * 5 * 5
    ...
    def _MCAU(self, ori_fea, channel_index):
        channel_mask = tf.expand_dims(tf.constant(
            CHANNEL_MASK[channel_index], dtype=tf.float32), axis=0)

        encoder_Q = tf.matmul(tf.reshape(ori_fea, (-1, CHANNEL_CNT)) * channel_mask,
                              self.encoder_Q_channel_weights['channel_%d' % channel_index])  # (batch * 5) * 4
        encoder_K = tf.matmul(tf.reshape(ori_fea, (-1, CHANNEL_CNT)) * channel_mask,
                              self.encoder_K_channel_weights['channel_%d' % channel_index])  # (batch * 5) * 4
        encoder_V = tf.matmul(tf.reshape(ori_fea, (-1, CHANNEL_CNT)) * channel_mask,
                              self.encoder_V_channel_weights['channel_%d' % channel_index])  # (batch * 5) * 4

        print('--- encoder_V ', encoder_V)
        encoder_Q = tf.reshape(encoder_Q,
                               (tf.shape(ori_fea)[0], ACTION_LEN, CHANNEL_CNT))  # batch * 5 * 4
        ...
        # 加mask
        print('--- ori_fea ', ori_fea)
        key_masks = tf.sign(tf.abs(tf.reduce_sum(
            tf.reshape(ori_fea, (-1, ACTION_LEN, CHANNEL_CNT)), axis=-1)))  # batch * 5
        key_masks = tf.tile(tf.expand_dims(key_masks, 1),
                            [1, ACTION_LEN, 1])  # batch * 5 * 5
        print('--- key_masks ', key_masks)
        attention_map = tf.matmul(encoder_Q, tf.transpose(
            encoder_K, [0, 2, 1]))  # batch * 5 * 5

        attention_map = attention_map / 8
        paddings = tf.ones_like(attention_map) * (-2 ** 32 + 1)
        attention_map = tf.where(
            tf.equal(key_masks, 0), paddings, attention_map)
    ...
    def _V_network(self, ads_emb, ois_emb):
        ...
        vnet_input = tf.reshape(
            tf.concat([ads_emb, ois_emb], axis=1), [-1, 40])
